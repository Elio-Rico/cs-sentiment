...
fig <- fig %>% add_trace(x = ~fdat$date, y = ~fdat$sentstandard, name = "Sentiment on Twitter", yaxis = "y2", mode = "lines+markers", type = "scatter", marker=list(color = "black", symbol = 'triangle-up',
                                      size = 3,
                                      opacity = 0.5, line = 
                                        list(width = 1, color = "black")),
...
<p class="bottom-three">
  ...
  In the plot, the sentiment measure represents the average sentiment per day of all collected tweets. The sentiment is then weighted by the number of tweets collected that day, divided by the total number of tweets. The measure is normalized (mean subtracted and divided by its standard deviation).^[The mean and standard deviation are calculated without observations of March 2023.] To improve the readability of the chart, missing values of daily returns over the weekend are linearly interpolated. Higher values correspond to a more positive sentiment, whereas lower values represent a negative sentiment. The correlation between the two series is `r correlation`.
